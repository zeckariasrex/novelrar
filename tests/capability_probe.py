...
This probe never supplies a password and never selects the optional py7zr
backend. Encrypted fixtures must stay REFUSED here. Supplied-password
HOST/OPTIONAL extraction is covered by tests/test_access.py.
...
def main() -> int:
    ...
    lines = [
        "# Capability matrix",
        "",
        "Generated by `tests/capability_probe.py`. Default probe: no password,",
        "no optional backend. Every `exact` count is a byte-for-byte comparison",
        "against the payload the fixture was built from. `REFUSED` is required",
        "for encrypted fixtures on this unattended path. Supplied-password",
        "HOST/OPTIONAL extraction is a separate caller path, not this probe.",
        "",
